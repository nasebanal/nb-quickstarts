...
- **Long-running services** (the `build`/`up`/`down`/`status`/`restart`/`open` pattern): Kong (gateway), Microcks (contract mock), agentgateway (MCP/A2A gateway), Keycloak (OIDC identity provider), Vault (secret storage). These run `docker compose up -d`, so they follow the same convention as every other module.
  ...
  - **Keycloak** and **Vault** are the only two test/verification modules that reach into `apps/backend`'s own Python code rather than treating it as a black box the way Kong/Consul/agentgateway do - a deliberate exception, since "does the real backend accept a token from a real IdP" and "does the real backend actually source its DB credential from a real secret store at startup" can't be demonstrated any other way. Both default to fully off (`KEYCLOAK_ISSUER`/`VAULT_ADDR`+`VAULT_TOKEN` all empty), so a checkout with neither module ever started behaves byte-for-byte as before either existed.
    - **Keycloak** (`quay.io/keycloak/keycloak`, `start-dev --import-realm`) imports a fixed realm on every start from `keycloak/realm/nasebanal-realm.json` (realm `nasebanal`, public client `apps-demo` with direct-access-grants enabled - no client secret needed for the password-grant demo flow, and no interactive browser redirect either - and one demo user, `keycloak-demo`/`nasebanal-demo`) - no persistent volume declared, so there's nothing to `reset` beyond a `restart` (see "Persistent state and `reset` targets" above). `app/auth.py`'s `get_current_username` (the same dependency every protected route already used) now tries a token as a Keycloak JWT first - via `PyJWKClient` against the realm's live JWKS - and only falls through to the legacy opaque-token dict lookup if that fails (or `KEYCLOAK_ISSUER` isn't set at all, or the token has no `.` in it - a legacy token is a bare `secrets.token_urlsafe()` string, never JWT-shaped, so it skips the JWT branch immediately, not even one wasted network call). `make keycloak:verify-apps` gets a real token from Keycloak and `POST`s it straight to the real `/accounts` - the exact route `POST /auth/login`-issued tokens already protect.
      - **Both `keycloak:login` and `keycloak:verify-apps` request the token from inside `apps-network`** (`docker run --rm --network apps-network curlimages/curl ...`, the same technique `consul:verify-apps` uses to reach container-only hostnames), not from the host at `localhost:$KEYCLOAK_PORT` - this isn't just a convenience, it's required for correctness. Keycloak's issued `iss` claim reflects whatever `Host` header the token request itself used, and `apps/backend` (also on `apps-network`) validates against `KEYCLOAK_ISSUER`'s in-network origin (`http://keycloak:8080/realms/nasebanal`) - a token requested from the host would carry a `localhost:8180` issuer that the backend would then correctly reject as untrusted. Issuing and validating against the exact same origin sidesteps the mismatch entirely rather than working around it with `KC_HOSTNAME`/hostname-strict configuration.
    - **Vault** (`hashicorp/vault`, dev-mode server - `VAULT_DEV_ROOT_TOKEN_ID` sets a fixed root token, auto-unseals, mounts KV v2 at `secret/`) holds `secret/apps/mysql` - the same MySQL user/password `apps/docker-compose.yml` already configures via `LOCUST_MYSQL_USER`/`LOCUST_MYSQL_PASSWORD`, written there by `make vault:put-mysql-secret`. Dev mode is in-memory only (no volume, same "no persistent state" shape as Keycloak above - and Microcks/locust). `app/config.py`'s `Settings` now tries `_fetch_vault_mysql_secret()` at import time and prefers its `user`/`password` over the plain `MYSQL_USER`/`MYSQL_PASSWORD` env vars whenever both `VAULT_ADDR` and `VAULT_TOKEN` are set and the fetch succeeds - using stdlib `urllib`, not `requests`, to avoid adding a dependency for one startup-time GET. `make vault:verify-apps` writes the secret, then recreates the real `backend` container with `VAULT_ADDR=http://vault:8200`/`VAULT_TOKEN=...` set for that one recreate only (not written to `.env`), and proves it actually took effect two ways: a `[vault] loaded MySQL credentials from ...` line in `docker logs nb-backend` (`print()`, not `logging` - nothing in this app configures a root handler, so a plain `logger.info()` call here would silently go nowhere and the whole point of this target is a log line you can actually `grep`), and a real `GET /accounts/balances` query succeeding against the connection opened with that credential - not just "the container is up," an actual MySQL round-trip.
