...
- **Several backends (Consul)**: the number of instances is an **apps** setting, `APPS_BACKEND_INSTANCES` (default 1), applied by `apps/bin/compose.sh` (wraps `docker compose -p apps`; `apps/Makefile` up/down/status/reset use it). On `up` it **generates** `apps/.backends.generated.yml` (gitignored) defining `backend-2..backend-N` by `extends`-ing apps' own `backend` service (same image, bind mount and environment; `build: !reset null` and `ports: !reset []` since apps already builds the image and only instance 1 publishes 8080), and passes `--remove-orphans` so a smaller N removes surplus containers. apps' `backend` gets `INSTANCE_ID: backend-1`; each instance adds it as an `X-Served-By` response header (`app/main.py`'s `ServedByMiddleware` - a raw ASGI middleware on purpose, since `@app.middleware("http")` wraps every request in `BaseHTTPMiddleware` and this app is what the load-test scenarios measure). `consul/bin/register_backends.sh` (via `make consul:register-apps`) finds the running apps backend containers by compose labels (`backend`, `backend-N`), registers each under the service name `apps-backend` with ID `apps-backend-N` (`Meta.instance`, a 2s HTTP check), deregisters stale `apps-backend*` entries and waits for N passing; it also registers `apps-mysql`. `consul:deregister-apps` removes all `apps-*`. `consul/bin/lb_demo.py` (stdlib only, run by `make consul:lb-demo` from a throwaway container on `apps-network`) is the client-side load balancer: per request it asks `/v1/health/service/apps-backend?passing`, takes the instances round robin, retries the next on a failure, and reports who answered from `X-Served-By`. Verified 3 -> 5 -> 2 -> 3, with `consul:lb-demo` spreading over exactly what was registered each time.
  ...
  - **Frontend server resolver** (`apps/frontend/src/lib/server/backendResolver.ts`, `app/api/backend/[...path]/route.ts`, `app/api/resolver/route.ts`): with `NEXT_PUBLIC_API_BASE=/api/backend` the browser calls the Next.js server, which forwards to a backend resolved in one of two modes - `direct` (`BACKEND_DIRECT_URL`, default `http://backend:8080`) or `consul` (`CONSUL_HTTP_ADDR`, 1s cache of the healthy `apps-backend` instances, round robin, falls back to the next on a network error). Mode is a runtime setting (`GET/PUT /api/resolver`; `BACKEND_RESOLVER` is only the initial mode). Responses carry `x-resolved-via` and `x-upstream`; failure is a 502 JSON. The accounts page (`components/BackendRouting.tsx`, `lib/servedBy.ts`) shows the answering instance and the switch. Verified: direct mode returns 502 once backend-1 is stopped, consul mode keeps answering from the others. Consul health-checks the backends and MySQL; the frontend's server (not the browser) is the one that asks - the overview's `consul` diagram variant draws exactly that.
  - **Overload comparison** (600 users / spawn 200 / 60s, `locustfile_http_overload_consul.py` discovers instances via Consul): one instance completed only 12 logins (median 5.1s, no counted failures since requests were still waiting), three completed 600 logins + 570 `POST /accounts`, 0 failures. The scenario-2 failure rates for one backend predate password login (PBKDF2 CPU cost) and may not reproduce - re-measure before quoting them together.
  - Not done, not verified: Kong resolving its upstream through Consul DNS (needs Kong's `dns_resolver` pointed at Consul). The Consul UI's instances tab kept showing "All service checks passing" for a stopped instance in headless captures even though `/v1/health/service` reported `critical`, so the docs show the failure with `make consul:instances` output instead of a UI screenshot.
