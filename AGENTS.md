...
Two more gaps, both fixed in `specmatic/docker-compose.yml` so `specmatic:test`
passes cleanly:

- **`/graphql` is dropped from Specmatic's copy** (`jq '... | del(.paths."/graphql")'`,
  same command as the version relabel above). Strawberry's `GraphQLRouter`
  publishes a `POST /graphql` OpenAPI entry with no `requestBody` schema at
  all — GraphQL's request shape (`{query, variables}`) isn't something
  OpenAPI can describe from introspection — so Specmatic has nothing to
  generate a body from and always gets a 400 "Unsupported content type"
  back. This isn't fixable by adding an example; GraphQL already has its own
  contract test (pytest's `test_graphql_query_and_mutation`), and testing an
  OpenAPI-shaped stand-in for it can't work by construction.
- **Two requests need real values Specmatic can't derive from the schema
  alone**, fixed with externalized examples (`--examples=`) built fresh on
  every run, in the same shell script:
  - `POST /accounts` needs a real bearer token. Specmatic *does* see that
    the route requires `Authorization` (it's declared in
    `components.securitySchemes`), but has no way to know what value would
    actually be accepted — that's a runtime secret (`/auth/login` issues a
    random token per call, held only in the backend's in-memory `_TOKENS`
    dict), not something derivable from the schema. Note: Specmatic's own
    `specmatic.yaml`-based `security.OpenAPI.securitySchemes` config exists
    for exactly this and does parse correctly (confirmed via the CLI's own
    error messages while getting the shape right - the class is
    `io.specmatic.core.BearerSecuritySchemeConfiguration`, `type: bearer` +
    `token: ...`, nested under `security.OpenAPI.securitySchemes.<name>`),
    but empirically had **no effect** on `specmatic test`'s generated
    requests even with a real, freshly-issued token wired up - still 401.
    Dropped in favor of an externalized example with the token in its
    `Authorization` header, which reliably works.
  - `GET /accounts/{account_id}` needs an id that actually exists. Left to
    ...
    `examples: [1]` on the path param in the OpenAPI schema itself (FastAPI:
    `account_id: int = Path(examples=[1])` in `routers/accounts.py`) is not
    enough on its own — Specmatic only actually uses an id value from an
    externalized example, not from the schema's own declared `examples`.
...
- **Three more examples cover the `422`s** FastAPI auto-documents for every
  route with a request body or a typed path param (Pydantic validation
  errors) - `POST /auth/login`, `POST /accounts`, and
  `GET /accounts/{account_id}`, each sent with deliberately invalid input
  (an empty body, or a non-numeric id) and its real error response fetched
  live and reused as the example's expected body, same pattern as the rest.
  Without these Specmatic's own coverage report listed all three as "not
  covered" even though the happy-path contract was fully verified - adding
  the negative example for each is enough on its own; no need for
  Specmatic's `SPECMATIC_GENERATIVE_TESTS` env var (which generates
  negative/mutated requests automatically). That was tried first and
  rejected: it also generates extra requests straight from the schema that
  bypass every externalized example above, bringing back the same
  401/404 problems those exist to solve, and new failures on top - our
  OpenAPI never formally declares `401` as a possible response for any
  route, so Specmatic can't accept an unexpected-but-correct 401 as valid
  either. Explicit negative examples get full coverage without any of that.

All example files are generated by the same shell script, not checked in -
the bearer token changes every run, and every expected response body is
fetched live from the backend so none of them can drift from reality.
`specmatic:test` is a clean, 100%-coverage pass as a result: 9 scenarios,
9 successes.
